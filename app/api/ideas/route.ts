...
export async function POST(request: Request) {
  ...
  try {
    ...
    const safeProviderCode=error instanceof Error&&/^provider-(http-\d{3}|config-invalid|incomplete|empty|refusal|too-large|malformed)$/.test(error.message)?error.message:undefined;
