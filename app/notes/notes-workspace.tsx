...
import { Archive, ExternalLink, ImagePlus, Lightbulb, Link2, LoaderCircle, NotebookPen, Pencil, Plus, Search, StickyNote, Trash2, X } from 'lucide-react';
...
function Editor({ note, onChange, onArchive, onDelete, onClose }: { note: Note; onChange: (change: Partial<Note>) => void; onArchive: () => void; onDelete: () => void; onClose: () => void }) {
  ...
  return <section className="nh-editor surface">
    ...
    <div className="nh-guided"><label>{template.context}<textarea rows={3} value={note.context} onChange={event => onChange({ context: event.target.value.slice(0, 4000) })}/></label><label>{template.body}<textarea rows={4} value={note.body} onChange={event => onChange({ body: event.target.value.slice(0, 8000) })}/></label><label>{template.outcome}<textarea rows={3} value={note.outcome} onChange={event => onChange({ outcome: event.target.value.slice(0, 4000) })}/></label><label>{template.next}<textarea rows={3} value={note.nextStep} onChange={event => onChange({ nextStep: event.target.value.slice(0, 3000) })}/></label></div>
    <section className="nh-attachments">
      ...
      <form onSubmit={event => { event.preventDefault(); const url = safeUrl(link.url); if (!url) { setError('http:// veya https:// ile başlayan geçerli bir bağlantı gir.'); return; } onChange({ links: [...note.links, { id: crypto.randomUUID(), title: link.title.trim() || new URL(url).hostname, url }] }); setLink({ title: '', url: '' }); }}><input aria-label="Bağlantı başlığı" value={link.title} onChange={event => setLink({ ...link, title: event.target.value })} placeholder="Bağlantı başlığı"/><input aria-label="Bağlantı adresi" type="url" value={link.url} onChange={event => setLink({ ...link, url: event.target.value })} placeholder="https://…"/><button disabled={!link.url.trim()}><Plus size={14}/> Ekle</button></form>
      <div className="nh-links">{note.links.map(item => <span key={item.id}><a href={safeUrl(item.url) || undefined} target="_blank" rel="noopener noreferrer"><ExternalLink size={13}/>{item.title}</a><button type="button" aria-label={`${item.title} bağlantısını kaldır`} onClick={() => onChange({ links: note.links.filter(linkItem => linkItem.id !== item.id) })}><Trash2 size={12}/></button></span>)}</div>
      <div className="nh-images">{note.images.map(item => <figure key={item.id}><img src={item.url} alt={item.caption || item.name}/><input aria-label={`${item.name} açıklaması`} value={item.caption} onChange={event => onChange({ images: note.images.map(image => image.id === item.id ? { ...image, caption: event.target.value.slice(0, 500) } : image) })} placeholder="Bu görsel neden burada?"/><button type="button" aria-label={`${item.name} görselini kaldır`} onClick={() => onChange({ images: note.images.filter(image => image.id !== item.id) })}><Trash2 size={13}/></button></figure>)}</div>
    </section>
    <footer><span>{note.date}</span><div className="nh-editor-actions"><button type="button" className="danger" onClick={onDelete}><Trash2 size={14}/> Notu sil</button><button type="button" onClick={onArchive}><Archive size={14}/> Arşivle</button></div></footer>
  </section>;
}

export function NotesWorkspace({ notes, onChange, onArchive, onDelete }: { notes: Note[]; onChange: (notes: Note[]) => void; onArchive: (note: Note) => void; onDelete: (note: Note) => void }) {
  ...
  const remove = (note: Note) => { if (window.confirm(`“${note.title}” notu kalıcı olarak silinsin mi?`)) { onDelete(note); if (selected === note.id) setSelected(null); } };
  return <div className="nh-shell">
    ...
    {active && <Editor key={active.id} note={active} onClose={() => setSelected(null)} onArchive={() => { onArchive(active); setSelected(null); }} onDelete={() => remove(active)} onChange={change => onChange(notes.map(note => note.id === active.id ? { ...note, ...change } : note))}/>}
    <div className="notes-toolbar"><div className="search-field"><Search size={15}/><input aria-label="Notlarda ara" placeholder="Başlık, içerik veya bağlam ara…" value={search} onChange={event => setSearch(event.target.value)}/>{search && <button aria-label="Not aramasını temizle" onClick={() => setSearch('')}><X size={13}/></button>}</div><div className="note-filters">{([['all', 'Tümü'], ['quick', 'Hızlı'], ['idea', 'Fikir'], ['journal', 'Günlük'], ['reference', 'Referans']] as const).map(([id, label]) => <button key={id} className={filter === id ? 'active' : ''} onClick={() => setFilter(id)}>{label}</button>)}</div></div>
    <div className="notes-grid">{filtered.map(note => { const TemplateIcon = templates[note.kind].icon; return <article key={note.id} role="button" tabIndex={0} aria-label={`${note.title} notunu aç`} className={`note-card surface ${note.tone}`} onClick={() => setSelected(note.id)} onKeyDown={event => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); setSelected(note.id); } }}><div className="note-card-top"><span><TemplateIcon size={12}/> {templates[note.kind].label.toLocaleUpperCase('tr-TR')}</span><div className="note-card-actions"><button type="button" title="Düzenle" aria-label={`${note.title} notunu düzenle`} onClick={event => { event.stopPropagation(); setSelected(note.id); }}><Pencil size={14}/></button><button type="button" title="Arşivle" aria-label={`${note.title} notunu arşivle`} onClick={event => { event.stopPropagation(); onArchive(note); }}><Archive size={15}/></button><button type="button" className="danger" title="Sil" aria-label={`${note.title} notunu sil`} onClick={event => { event.stopPropagation(); remove(note); }}><Trash2 size={15}/></button></div></div><h2>{note.title}</h2><p>{note.body || note.context || 'Bu not henüz tamamlanmadı.'}</p><div className="nh-card-meta">{note.links.length > 0 && <span><Link2 size={11}/>{note.links.length}</span>}{note.images.length > 0 && <span><ImagePlus size={11}/>{note.images.length}</span>}</div><footer><span>{note.date}</span><button type="button" onClick={event => { event.stopPropagation(); setSelected(note.id); }}><Pencil size={12}/> Düzenle</button></footer></article>; })}</div>
