...
import { ArrowRight, Check, Copy, History, Plus, RotateCcw, Sparkle } from 'lucide-react';
...
export function IdeaStudio({request,onSave,onClose,historyOnly=false}:{request:Omit<IdeaRequest,'signal'>;onSave:(idea:GeneratedIdea)=>void|Promise<void>;onClose:()=>void;historyOnly?:boolean}) {
  ...
  const project=idea?.type==='project'||idea?.type==='digital_project';
  const visual=idea?.type==='image_prompt',concept=visual&&idea.visualMode==='concept';
  const platforms={mobile_app:'MOBILE',web_app:'WEB',desktop_app:'DESKTOP',game:'GAME',browser_extension:'BROWSER',plugin:'PLUGIN',automation:'AUTOMATION',interactive_experience:'INTERACTIVE'};
  ...
  return <>
    ...
    {showHistory?<div className="rd-generation-history"><h2>Üretim geçmişi.</h2><p>Önceden üretilenler. Bunlar yeni öneriler olarak sunulmaz.</p>
      {history.map(item=><button key={item.id} onClick={()=>{setIdea(item);setFromHistory(true);setShowHistory(false);setError('');setCopied(false);}}><span><strong>{item.title}</strong><small>{new Date(item.generatedAt).toLocaleDateString('tr-TR')} · {item.type==='image_prompt'?(item.visualMode==='concept'?'Konsept':item.visualMode==='variation'?'Varyasyon':'Görsel promptu'):item.status==='accepted'?'Kaydedildi':item.status==='skipped'?'Geçildi':item.status==='rejected'?'Reddedildi':'Üretildi'}</small></span><ArrowRight size={17}/></button>)}
    ...
    </div>:<div className={`rd-idea-stage ${busy?'is-drawing':''} ${visual&&!concept?'rd-prompt-stage':''}`} aria-live="polite" aria-busy={busy||saving}>
      {idea&&<div className="rd-drawn-idea" key={idea.id}><span className="rd-kicker">{visual?concept?'VISUAL LAB / KONSEPT':'VISUAL LAB / PROMPT':idea.platform?platforms[idea.platform]:idea.type==='meal'?'NE YESEM?':idea.kind} · {idea.domain}</span>{visual&&!concept?<><h2>{idea.title}</h2><textarea ref={promptField} readOnly aria-label="Görsel oluşturma promptu" value={idea.text}/><p>Prompt geçmişe kaydedildi. İstediğin görsel üretim aracında kullanabilirsin.</p></>:<h2>{idea.text}</h2>}{saving&&<p role="status">{project?'Bu fikre özel proje planı hazırlanıyor…':idea.type==='research'?'Bu konuya özel sorular hazırlanıyor…':'Kaydediliyor…'}</p>}</div>}
