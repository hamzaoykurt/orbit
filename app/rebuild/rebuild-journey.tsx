...
export function RebuildJourney({journey,deck,activities,selections,syncStatus,practice,linkedProject,fitness,onUpdateDeck,onStartChange,onUpdatePractice,onCreateProject,onOpenProject,onCopyResearch}:Props) {
  ...
  const rowHeading=(name:string,title:string,summary:ReactNode)=><button className="rd-domain-toggle" aria-expanded={expanded===name} aria-controls={`rd-detail-${name}`} onClick={()=>toggle(name)}><span className="rd-domain-title">{title}</span><span className="rd-domain-summary">{summary}</span></button>;
  return <section className="rebuild-deck rd-instrument" aria-label="Rebuild haftalık alanı">
    ...
    <div className="rd-list-tools"><span>KENDİ RİTMİNDE</span><button className="rd-icon" aria-label="Haftalık hedefleri düzenle" onClick={()=>{setGoalDraft(week.goals.filter(goal=>['english','social'].includes(goal.kind)).map(goal=>({...goal})));setOverlay('settings');}}><Settings2 size={17}/></button></div>
    <div className="rd-domains">
      ...
      <section className={`rd-domain rd-sport ${expanded==='body'?'is-open':''}`}>
        <div className="rd-domain-line">{rowHeading('body','Spor',fitness.connected&&fitness.entitled?<><Dots count={sportCount} target={sportTarget}/><span>{sportCount} / {sportTarget}</span></>:<span>Fitness özeti</span>)}</div>
        <div className="rd-row-action"><FitnessLink/></div>
        {expansion('body',<><p className="rd-inline-copy">{fitness.status==='loading'?'Fitness özeti yükleniyor…':fitness.status==='error'?'Son Fitness özeti şu anda yenilenemedi. Fitness kayıtların etkilenmedi.':!fitness.connected?'Fitness bağlantısı isteğe bağlıdır. Bağlandığında ilerleme burada otomatik görünür.':!fitness.entitled?'Orbit Premium Fitness Sync bu hesapta etkin değil.':syncedSport?`Bu hafta Fitness’ta ${sportCount} / ${sportTarget} gerçek seans tamamlandı. Tamamlama ve düzeltmeler yalnız Fitness’tan gelir.`:'Bağlantı hazır. İlk güvenli Fitness özeti bekleniyor.'}</p>{practice.lastMeal&&<p className="rd-inline-copy"><strong>Yemek fikrin</strong><br/>{practice.lastMeal.text}</p>}<div className="rd-detail-actions"><button className="rd-text-button" onClick={()=>openIdea({type:'meal',goal:'body'})}>Ne yesem? <Sparkle size={15}/></button></div></>)}
      </section>
      ...
      <section className={`rd-domain rd-social-row ${expanded==='social'?'is-open':''}`}>
        <div className="rd-domain-line">{rowHeading('social','Social',<><Dots count={socialCount} target={social?.target||1}/><span>{socialCount} / {social?.target||1}</span></>)}</div>
        {expansion('social',<><p className="rd-inline-copy">{social&&week.ideas[social.id]?week.ideas[social.id].text:'Açık hava, kapalı alan, dijital, kültür veya topluluk: bu hafta biçimi sen seç.'}</p><IdeaFilter label="Etkinlik türü" value={socialCategory} options={SOCIAL_CATEGORIES} onChange={value=>setSocialCategory(value as SocialCategory)}/>{social&&<div className="rd-completion-panel"><button className="rd-completion-action" disabled={socialCount>=social.target} onClick={()=>done(social)}><Check size={16}/>{socialCount>=social.target?'Bu haftanın hedefi tamamlandı':'Aksiyonu tamamlandı olarak işaretle'}</button>{socialCount>0&&<button className="rd-text-button" onClick={()=>undo(social)}><RotateCcw size={15}/> Son işareti geri al</button>}</div>}<div className="rd-detail-actions"><button className="rd-text-button" onClick={()=>openIdea({type:'activity',goal:'social',category:socialCategory})}>Bu türde fikir ver <Sparkle size={15}/></button></div></>)}
      </section>
      ...
      <div className="rd-domain-group rd-explore-group">
        ...
        <section className={`rd-domain rd-research-row ${expanded==='research'?'is-open':''}`}>
          ...
          <div className="rd-row-action">{topic?<button className="rd-text-button" onClick={()=>toggle('research')}>Devam et <ArrowRight size={15}/></button>:<button className="rd-text-button" onClick={()=>openIdea({type:'research',goal:'research',category:researchCategory})}>Bana bir konu ver <Sparkle size={15}/></button>}</div>
          {expansion('research',<><IdeaFilter label="Konu kategorisi" value={researchCategory} options={RESEARCH_CATEGORIES} onChange={value=>setResearchCategory(value as ResearchCategory)}/>{topic&&<ResearchNotebook key={topic.id} topic={topic} onChange={change=>onUpdatePractice(current=>({...current,research:current.research.map(item=>item.id===topic.id?change(item):item)}))}/>}<div className="rd-detail-actions">{topic&&<button className="rd-text-button" onClick={()=>onCopyResearch(topic)}>Araştırma bağlamını kopyala</button>}<button className="rd-text-button" onClick={()=>openIdea({type:'research',goal:'research',category:researchCategory})}>{topic?'Bu kategoride yeni konu':'Konu üret'} <Sparkle size={15}/></button><button className="rd-text-button" onClick={()=>setOverlay('history')}><History size={15}/> Araştırma geçmişi</button></div></>)}
        </section>
        <section className={`rd-domain rd-create-row ${expanded==='make'?'is-open':''}`}>
          ...
          <div className="rd-row-action">{linkedProject?<button className="rd-text-button" onClick={()=>onOpenProject(linkedProject.id)}>Projeyi aç <ArrowUpRight size={16}/></button>:<button className="rd-text-button" onClick={()=>openIdea({type:'project',goal:'make',category:createCategory})}>Bana bir proje ver <Sparkle size={15}/></button>}</div>
          {expansion('make',<><p className="rd-inline-copy">Yazılım, fiziksel üretim, elektronik, görsel, el işi veya deneysel bir nesne. Alanı seçebilir ya da karışık bırakabilirsin.</p><IdeaFilter label="Üretim alanı" value={createCategory} options={CREATE_CATEGORIES} onChange={value=>setCreateCategory(value as CreateCategory)}/><div className="rd-detail-actions"><button className="rd-text-button" onClick={()=>openIdea({type:'project',goal:'make',category:createCategory})}>{linkedProject?'Başka bir proje fikri':'Bu alanda fikir üret'} <Sparkle size={15}/></button><button className="rd-text-button" onClick={()=>openIdea({type:'project'},true)}><History size={15}/> Create geçmişi</button></div></>)}
        </section>
        <section className={`rd-domain rd-digital-row ${expanded==='digital'?'is-open':''}`}>
          ...
          <div className="rd-row-action"><button className="rd-text-button" onClick={()=>openIdea({type:'digital_project',goal:'make',...(digitalPlatform==='surprise'?{}:{platform:digitalPlatform})})}>Bana bir dijital proje ver <Sparkle size={15}/></button></div>
          {expansion('digital',<><p className="rd-inline-copy">Mobil, web, masaüstü, oyun, otomasyon ve interaktif deneyimler. Platform seçimi fikrin mekaniğini de değiştirir.</p><IdeaFilter label="Dijital platform" value={digitalPlatform} options={DIGITAL_PLATFORMS} onChange={value=>setDigitalPlatform(value as 'surprise'|DigitalPlatform)}/><div className="rd-detail-actions"><button className="rd-text-button" onClick={()=>openIdea({type:'digital_project',goal:'make',...(digitalPlatform==='surprise'?{}:{platform:digitalPlatform})})}>Bu platformda fikir üret <Sparkle size={15}/></button><button className="rd-text-button" onClick={()=>openIdea({type:'digital_project'},true)}><History size={15}/> Digital geçmişi</button></div></>)}
        </section>
