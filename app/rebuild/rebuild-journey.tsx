  ...
  return <section className="rebuild-deck rd-instrument" aria-label="Rebuild haftalık alanı">
    ...
    <div className="rd-domains">
      ...
      <section className={`rd-domain rd-social-row ${expanded==='social'?'is-open':''}`}>
        ...
        {expansion('social',<><p className="rd-inline-copy">{social&&week.ideas[social.id]?week.ideas[social.id].text:'Bu hafta alıştığın rutinin dışında bir şey yap.'}</p>{social&&<div className="rd-completion-panel"><button className="rd-completion-action" disabled={socialCount>=social.target} onClick={()=>done(social)}><Check size={16}/>{socialCount>=social.target?'Bu haftanın hedefi tamamlandı':'Aksiyonu tamamlandı olarak işaretle'}</button>{socialCount>0&&<button className="rd-text-button" onClick={()=>undo(social)}><RotateCcw size={15}/> Son işareti geri al</button>}</div>}<div className="rd-detail-actions"><button className="rd-text-button" onClick={()=>openIdea({type:'activity',goal:'social'})}>Bana bir fikir ver <Sparkle size={15}/></button></div></>)}
      </section>
