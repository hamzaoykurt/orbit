  ...
  return <section className="rebuild-deck rd-instrument" aria-label="Rebuild haftalık alanı">
    ...
    <div className="rd-domains">
      ...
      <section className={`rd-domain rd-sport ${expanded==='body'?'is-open':''}`}>
        <div className="rd-domain-line">{rowHeading('body','Spor',<><Dots count={sportCount} target={sport?.target||3}/><span>{sportCount} / {sport?.target||3}</span></>)}</div>
        <div className="rd-row-action"><FitnessLink/></div>
        {expansion('body',<><p className="rd-inline-copy">Programın ve hareketlerin Fitness’te. Burada yalnızca tamamladığın seansı işaretle.</p>{sport&&<div className="rd-completion-panel"><button className="rd-completion-action" disabled={sportCount>=sport.target} onClick={()=>done(sport)}><Check size={16}/>{sportCount>=sport.target?'Bu haftanın hedefi tamamlandı':'Seansı tamamlandı olarak işaretle'}</button>{sportCount>0&&<button className="rd-text-button" onClick={()=>undo(sport)}><RotateCcw size={15}/> Son işareti geri al</button>}</div>}{practice.lastMeal&&<p className="rd-inline-copy"><strong>Yemek fikrin</strong><br/>{practice.lastMeal.text}</p>}<div className="rd-detail-actions"><button className="rd-text-button" onClick={()=>openIdea({type:'meal',goal:'body'})}>Ne yesem? <Sparkle size={15}/></button></div></>)}
      </section>
