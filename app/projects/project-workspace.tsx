...
import { ArrowLeft, CalendarDays, Check, ChevronDown, ClipboardCopy, ExternalLink, ImagePlus, LayoutList, Link2, Network, Pencil, Plus, StickyNote, Trash2 } from 'lucide-react';
...
export function ProjectWorkspace(props: Props) {
  ...
  return <div className="project-workspace">
    <div className="pw-topbar"><button type="button" onClick={props.onBack}><ArrowLeft size={17}/> Proje panosu</button><span className={`pw-sync ${props.syncStatus === 'error' ? 'pw-error' : ''}`} role="status">{props.syncStatus === 'saved' ? 'Değişiklikler kaydedildi' : props.syncStatus === 'saving' ? 'Kaydediliyor…' : props.syncStatus === 'error' ? 'Sunucuya kaydedilemedi' : 'Veriler yükleniyor…'}</span><button type="button" className="pw-context-copy" onClick={props.onCopyContext}><ClipboardCopy size={15}/> Proje bağlamını kopyala</button>{props.syncStatus === 'error' && <button type="button" onClick={props.onRetry}>Tekrar dene</button>}</div>
    ...
    {tab === 'tasks' && <section className="pw-task-section"><div className="pw-section-heading"><div><h2>Bir sonraki adım.</h2><p>Görev aç, küçük adımlara böl, görselle anlat.</p></div><label>Görünüm<select value={filter} onChange={event => setFilter(event.target.value as typeof filter)}><option value="all">Tüm görevler</option><option value="open">Açık işler</option><option value="done">Tamamlananlar</option></select></label></div><AddLine label="Projeye yeni görev ekle…" onAdd={title => { props.onAddTask(title); setFilter('all'); }}/>
      <div className="pw-task-list">{visibleTasks.map(task => {
        ...
        return <article className="surface pw-task" key={task.id}><div className="pw-task-title"><button type="button" className={`pw-check ${completed[task.id] ? 'checked' : ''}`} aria-label={`${task.title}: ${completed[task.id] ? 'yeniden aç' : 'tamamla'}`} aria-pressed={!!completed[task.id]} onClick={() => props.onToggle(task.id)}>{completed[task.id] && <Check size={15}/>}</button><h3 className={completed[task.id] ? 'pw-done' : ''}>{task.title}</h3><button type="button" title="Kopyala" aria-label={`${task.title} görevini kopyala`} onClick={() => props.onCopyTask(taskCopyText({title:task.title,context:`Proje · ${project.title}`,status:completed[task.id]?'Tamamlandı':'Açık',note:details.note,subtasks:task.children.map(child=>({title:child.title,completed:!!completed[child.id]}))}))}><ClipboardCopy size={17}/></button><button type="button" title="Takvime ekle" aria-label={`${task.title} görevini takvime ekle`} onClick={() => props.onSchedule(task.title)}><CalendarDays size={17}/></button><button type="button" title="Görevi düzenle" aria-label={`${task.title} görevini düzenle`} onClick={() => props.onEditTask(task.index, task.title)}><Pencil size={17}/></button><button type="button" className="pw-task-delete" title="Görevi sil" aria-label={`${task.title} görevini sil`} onClick={() => { if (window.confirm(`“${task.title}” görevi ve bağlı alt görevleri silinsin mi?`)) props.onRemoveTask(task.index); }}><Trash2 size={17}/></button></div>
