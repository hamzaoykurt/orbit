...
export function ProjectWorkspace(props: Props) {
  ...
  return <div className="project-workspace">
    ...
    {tab === 'resources' && <div className="pw-resource-grid"><section className="surface pw-resource"><div className="pw-section-heading"><div><h2>Proje defteri</h2><p>Kararlar, fikirler ve çalışma notları.</p></div><button type="button" onClick={() => props.onWorkspace(current => ({ ...current, notes: [...current.notes, { id: crypto.randomUUID(), title: 'Yeni not', body: '' }] }))}><Plus size={16}/> Not ekle</button></div>{!workspace.notes.length && <p className="pw-hint">Proje için aldığın kararları burada biriktir.</p>}{workspace.notes.map(note => <div className="pw-note" key={note.id}><label>Not başlığı<input maxLength={160} value={note.title} onChange={event => props.onWorkspace(current => ({ ...current, notes: current.notes.map(item => item.id === note.id ? { ...item, title: event.target.value } : item) }))}/></label><label>İçerik<textarea maxLength={12000} rows={6} value={note.body} onChange={event => props.onWorkspace(current => ({ ...current, notes: current.notes.map(item => item.id === note.id ? { ...item, body: event.target.value } : item) }))} placeholder="Düşüncelerini buraya bırak…"/></label><button type="button" className="pw-delete" onClick={() => { if (window.confirm('Bu not silinsin mi?')) props.onWorkspace(current => ({ ...current, notes: current.notes.filter(item => item.id !== note.id) })); }}><Trash2 size={14}/> Notu sil</button></div>)}</section><section className="surface pw-resource"><h2>Bağlantı rafı</h2><p>Tasarım dosyaları, depolar, referanslar.</p><form className="pw-link-form" onSubmit={event => { event.preventDefault(); const url = safeResourceUrl(linkUrl); if (!url) { setError('https:// veya http:// ile başlayan geçerli bir bağlantı gir.'); return; } props.onWorkspace(current => ({ ...current, links: [...current.links, { id: crypto.randomUUID(), title: linkTitle.trim() || new URL(url).hostname, url }] })); setLinkTitle(''); setLinkUrl(''); setError(''); }}><label>Başlık<input maxLength={160} value={linkTitle} onChange={event => setLinkTitle(event.target.value)} placeholder="Örn. Figma tasarımları"/></label><label>Bağlantı<input type="url" required maxLength={2000} value={linkUrl} onChange={event => setLinkUrl(event.target.value)} placeholder="https://…"/></label><button><Link2 size={16}/> Bağlantı ekle</button></form>{workspace.links.map(link => <div className="pw-resource-link" key={link.id}><a href={safeResourceUrl(link.url) ?? undefined} target="_blank" rel="noopener noreferrer"><ExternalLink size={16}/><span><strong>{link.title}</strong><small>{link.url}</small></span></a><button type="button" aria-label={`${link.title} bağlantısını kaldır`} onClick={() => { if (window.confirm('Bu bağlantı kaldırılsın mı?')) props.onWorkspace(current => ({ ...current, links: current.links.filter(item => item.id !== link.id) })); }}><Trash2 size={14}/></button></div>)}</section></div>}
