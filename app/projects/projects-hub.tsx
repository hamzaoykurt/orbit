...
import { ArrowUpRight, Plus, Search, Lightbulb, ArrowRight, Layers, ClipboardCopy } from 'lucide-react';
...
export function ProjectsHub({projects,workspaces,hasDraft,onNew,onOpen,onCopy}:{projects:Metric[];workspaces:Record<string,ProjectWorkspaceData>;hasDraft:boolean;onNew:()=>void;onOpen:(id:string)=>void;onCopy:(id:string)=>void}) {
  ...
  return <div className="pp-root pp-hub"><header className="pp-hub-header"><div><span className="pp-kicker">KİŞİSEL PROJE ATÖLYEN</span><h1>Bir fikir. Gerçek bir başlangıç.</h1><p>Fikrini netleştir, karakterini bul, ilk denemeni yap.</p></div><button type="button" className="pp-button pp-primary" onClick={onNew}>{hasDraft?<ArrowRight size={18}/>:<Plus size={18}/>} {hasDraft?'Taslağa devam et':'Yeni proje'}</button></header>
    ...
    <div className="pp-project-grid">{visible.map(({project,progress,done,total})=>{
      ...
      return <article className="pp-project-card" key={project.id}><button type="button" className="pp-project-card-copy" title="Proje bağlamını kopyala" aria-label={`${project.title} proje bağlamını kopyala`} onClick={()=>onCopy(project.id)}><ClipboardCopy size={16}/></button><button type="button" className="pp-project-open" onClick={()=>onOpen(project.id)} aria-label={`${project.title} projesini aç`}>
