...
import { newCreationDraft, normalizeCreationDraft, normalizePlanning, lifecycleFromStage, stageFromLifecycle, lifecycleLabels } from './projects/planning-types';
...
import { designCatalog } from './projects/design-catalog';
import { findSimilar, mentorContext, parseMentorOutput, projectContext, researchContext } from './mentor-sync';
...
export default function PersonalOS() {
  ...
  const [mentorTab,setMentorTab]=useState<'context'|'import'>('context');
  const [mentorInput,setMentorInput]=useState('');
  const [mentorParsed,setMentorParsed]=useState<MentorImport|null>(null);
  const [mentorError,setMentorError]=useState('');
  const [mentorDuplicate,setMentorDuplicate]=useState<{id:string;title:string;kind:'project'|'research'}|null>(null);
  const [mentorPreview,setMentorPreview]=useState(false);
  const [mentorGeneratedAt,setMentorGeneratedAt]=useState<Date|null>(null);
  ...
  const mentorWeekStart=(()=>{const date=new Date();const day=(date.getDay()+6)%7;date.setDate(date.getDate()-day);return localDateKey(date);})();
  const mentorWeek=state.rebuildDeck.weeks[mentorWeekStart];
  const mentorProjectView=(project:Project):MentorProjectView=>{
    const tasks=visibleProjectTasks(project),workspace=state.projectWorkspaces[project.id]??emptyWorkspace;
    const completedTasks=tasks.filter((_,index)=>state.completed[`project-${project.id}-${index}`]);
    const remaining=tasks.filter((_,index)=>!state.completed[`project-${project.id}-${index}`]);
    const plan=workspace.planning;const stage=plan?lifecycleLabels[plan.lifecycle]:['Fikir','Aktif','Araştırma','Tamamlandı'][project.stage]||'Fikir';
    return {id:project.id,name:project.title,goal:workspace.description||plan?.input.answers.goal||'',stage,type:plan?.analysis.type||project.tags[0]||'',scope:(plan?.overrides.scope||plan?.analysis.scope||''),nextAction:remaining[0]||plan?.analysis.firstSteps[0]||'',progress:completionRate(completedTasks.length,tasks.length),tasks:remaining,completed:completedTasks,designLanguage:designCatalog.find(style=>style.id===plan?.selectedStyle)?.name||workspace.mentor?.suggestedDesignLanguage||'',notes:workspace.notes.slice(-3).map(note=>note.body||note.outcome||note.title),lastActivity:workspace.mentor?.importedAt||plan?.updatedAt||''};
  };
  ...
  const buildMentorContext=()=>{
    ...
    const count=(kind:string)=>mentorWeek?.goals.filter(goal=>goal.kind===kind).reduce((sum,goal)=>sum+(mentorWeek.marks[goal.id]?.length||0),0)||0;
  ...
  const previewMentorImport=()=>{
    ...
    if(!parsed){setMentorError('Could not recognize this Mentor output.');return;}
    const match=parsed.kind==='project'?findSimilar(parsed.name,metricProjects,p=>p.title):findSimilar(parsed.title,state.rebuildPractice.research,r=>r.title);
    if(match)setMentorDuplicate(parsed.kind==='project'?{id:(match as Project).id,title:(match as Project).title,kind:'project'}:{id:(match as typeof state.rebuildPractice.research[number]).id,title:(match as typeof state.rebuildPractice.research[number]).title,kind:'research'});
  };
  const importFromMentor=()=>{
    if(!mentorParsed)return;
    if(mentorParsed.kind==='research'){
      const item=mentorParsed,id=`mentor-research-${crypto.randomUUID()}`,at=new Date().toISOString();
      setState(current=>({...current,rebuildPractice:{...current.rebuildPractice,currentResearchId:id,research:[...current.rebuildPractice.research,{id,ideaId:'',source:'mentor',title:item.title,question:item.mainQuestion,startedAt:at,optionalOutput:item.optionalOutput,questions:item.subquestions.map((text,index)=>({id:`${id}-${index}`,text,explored:false,note:'',evidence:'',implication:'',unknown:''})),synthesis:{explanation:'',keyPoints:'',openQuestions:''},sources:[],images:[],quiz:null}]}}));
      setModal(null);setMentorInput('');setMentorParsed(null);go('rebuild');notify('Araştırma, mevcut Research sistemine eklendi.');return;
    }
    const item=mentorParsed,id=`mentor-project-${crypto.randomUUID()}`,now=new Date().toISOString();
    const lifecycle=(()=>{const s=(item.stage||'').toLocaleLowerCase('tr');if(/pause|bekle/.test(s))return 'paused' as const;if(/complete|done|tamam|archive/.test(s))return 'completed' as const;if(/research|araştır|explore|incele/.test(s))return 'research' as const;if(/mvp/.test(s))return 'mvp' as const;if(/active|build|develop|devam|geliştir/.test(s))return 'active' as const;return 'idea' as const;})();
    const draft:CreationDraft={id,title:item.name,idea:item.description||item.goal||item.name,answers:{goal:item.goal||'',scope:item.scope||'',tone:item.designLanguage||''},step:'review',question:0,selectedStyle:null,lifecycle};
    const analysis=analyzeProject(draft);if(item.type)analysis.type=item.type;if(item.scope)analysis.scope=item.scope;if(item.nextAction)analysis.firstSteps=[item.nextAction,...analysis.firstSteps.filter(x=>x!==item.nextAction)];
    const style=item.designLanguage?designCatalog.find(value=>[value.id,value.name].some(label=>label.toLocaleLowerCase('tr')===item.designLanguage!.toLocaleLowerCase('tr'))):undefined;
    const plan:ProjectPlanning={version:1,createdAt:now,updatedAt:now,input:{...draft,selectedStyle:style?.id||null},analysis,selectedStyle:style?.id||'',lifecycle,overrides:{...(item.type?{type:item.type}:{}),...(item.scope?{scope:item.scope}:{})}};
    const tasks=[...(item.nextAction?[item.nextAction]:[]),...item.tasks].filter((task,index,array)=>array.indexOf(task)===index).slice(0,40);
    const project:Project={id,title:item.name,stage:stageFromLifecycle(lifecycle),progress:0,color:'violet',cover:'minimal',due:'Tarihsiz',tags:item.tags.length?item.tags:item.type?[item.type]:[],tasks:tasks.length?tasks:analysis.firstSteps};
    let workspace=applyPlanning({...emptyWorkspace,description:item.description||item.goal||''},plan);workspace={...workspace,mentor:{source:'mentor',importedAt:now,...(!style&&item.designLanguage?{suggestedDesignLanguage:item.designLanguage}:{})}};
    setState(current=>({...current,customProjects:[...current.customProjects,project],projectStages:{...current.projectStages,[id]:project.stage},projectWorkspaces:{...current.projectWorkspaces,[id]:workspace}}));
    setModal(null);setMentorInput('');setMentorParsed(null);openProjectDetail(id);notify('Proje, görevleri ve başlangıç rotasıyla eklendi.');
  };
  ...
  const renderProjects = () => {
    ...
    if (activeProjectId) {
      ...
      return <Suspense fallback={<p role="status">Proje araçları yükleniyor…</p>}><ProjectWorkspace key={project.id} project={project} tasks={visibleProjectTasks(project)} subtasks={state.projectSubtasks} completed={state.completed} details={state.projectTaskDetails} workspace={state.projectWorkspaces[project.id] ?? emptyWorkspace} syncStatus={syncStatus} onRetry={() => setSyncRetry(value => value + 1)} onBack={() => backTo('projects')} onEdit={() => openProjectEdit(project)} onPlan={() => developProject(project)} onResearch={() => openProjectResearch(project)} onCopyContext={()=>void copyText(projectContext(mentorProjectView(project)),'Proje bağlamı kopyalandı.')} onToggle={toggle} onSchedule={title => scheduleItem(title, `Proje · ${project.title}`)}
  ...
  return <div className="app-shell">
    ...
    {modal&&<div className="modal-layer" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget){playFeedback('tap',true);setModal(null);}}}><section className={`modal-card ${modal}`} role="dialog" aria-modal="true" aria-label="Orbit penceresi" onKeyDownCapture={handleModalKeyDown} onChangeCapture={handleModalChange} onFocusCapture={handleModalFocus}><IconButton label="Kapat" className="modal-close" onClick={()=>setModal(null)}><X size={17}/></IconButton>
      {modal==='mentor'&&<><span className="modal-icon"><ClipboardCopy size={20}/></span><span className="eyebrow">MENTOR SYNC</span><h2>Mentor köprüsü</h2><p>Orbit durumunu dış mentoruna taşı veya mentorunun hazırladığı kaydı mevcut sistemine ekle.</p><div className="mentor-tabs" role="tablist"><button className={mentorTab==='context'?'selected':''} onClick={()=>setMentorTab('context')}><ClipboardCopy size={14}/> Bağlamı kopyala</button><button className={mentorTab==='import'?'selected':''} onClick={()=>setMentorTab('import')}><FileInput size={14}/> Mentor’dan içe aktar</button></div>{mentorTab==='context'?<div className="mentor-context"><small>Son oluşturma: {mentorGeneratedAt?mentorGeneratedAt.toLocaleString('tr-TR'):'Henüz oluşturulmadı'}</small><button className="primary-button full" onClick={()=>{const now=new Date();setMentorGeneratedAt(now);void copyText(buildMentorContext(),'Mentor bağlamı kopyalandı.');}}><ClipboardCopy size={15}/> Güncel bağlamı kopyala</button><button className="mentor-preview-toggle" onClick={()=>{setMentorPreview(value=>!value);if(!mentorGeneratedAt)setMentorGeneratedAt(new Date());}}>{mentorPreview?'Önizlemeyi kapat':'Önizle'}</button>{mentorPreview&&<pre className="mentor-preview">{buildMentorContext()}</pre>}</div>:<div className="mentor-import">{!mentorParsed?<><label>Mentor çıktısı<textarea rows={10} value={mentorInput} onChange={event=>{setMentorInput(event.target.value);setMentorError('');}} placeholder={'PROJECT\nname: Orbital Radio\nstage: explore\ntasks:\n- Akışı çiz\n- Prototipi hazırla'}/></label>{mentorError&&<div className="mentor-error" role="alert"><strong>{mentorError}</strong><small>PROJECT + name: … veya RESEARCH + title: … + main_question: … biçimini kullan.</small></div>}<button className="primary-button full" disabled={!mentorInput.trim()} onClick={previewMentorImport}>Önizle <ArrowRight size={15}/></button></>:<><div className="mentor-import-preview"><span>{mentorParsed.kind==='project'?'PROJE EKLE':'ARAŞTIRMA EKLE'}</span><h3>{mentorParsed.kind==='project'?mentorParsed.name:mentorParsed.title}</h3>{mentorParsed.kind==='project'?<><p>{mentorParsed.type||'Proje'} · {mentorParsed.stage||'Fikir'}</p><strong>{mentorParsed.tasks.length} görev</strong>{mentorParsed.designLanguage&&<small>Tasarım dili: {mentorParsed.designLanguage}</small>}</>:<><p>{mentorParsed.mainQuestion}</p><strong>{mentorParsed.subquestions.length} soru</strong></>}</div>{mentorDuplicate?<div className="mentor-duplicate"><strong>Benzer bir {mentorDuplicate.kind==='project'?'proje':'araştırma'} zaten var:</strong><p>{mentorDuplicate.title}</p><div><button onClick={()=>{setModal(null);if(mentorDuplicate.kind==='project')openProjectDetail(mentorDuplicate.id);else{setState(current=>({...current,rebuildPractice:{...current.rebuildPractice,currentResearchId:mentorDuplicate.id}}));go('rebuild');}}}>Mevcut kaydı aç</button><button onClick={()=>setMentorDuplicate(null)}>Yine de içe aktar</button><button onClick={()=>{setMentorParsed(null);setMentorDuplicate(null);}}>İptal</button></div></div>:<div className="mentor-confirm"><button onClick={()=>setMentorParsed(null)}>Geri dön</button><button className="primary-button" onClick={importFromMentor}>{mentorParsed.kind==='project'?'Projelere ekle':'Research’e ekle'} <Check size={15}/></button></div>}</>}</div>}</>}
