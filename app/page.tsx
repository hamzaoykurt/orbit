...
const defaultState: PersistedState = {
  ...
  settings: { notifications: true, motion: true, sound: false, autoArchive: true, accent: 'violet', theme: 'system' },
};
...
const calendarEvents: Record<number, { title: string; tone: string }[]> = {
  3: [{ title: 'İngilizce konuşma', tone: 'blue' }], 6: [{ title: 'Merak araştırması', tone: 'violet' }],
  10: [{ title: 'Spor · 45–60 dk', tone: 'mint' }], 14: [{ title: 'Sosyal ortam', tone: 'orange' }],
  18: [{ title: 'Orbit prototipi', tone: 'blue' }], 20: [{ title: 'Diksiyon kaydı', tone: 'rose' }],
  23: [{ title: 'Haftalık kayıt', tone: 'violet' }, { title: 'Solo çıkış', tone: 'sand' }],
  26: [{ title: 'Yaratıcı üretim', tone: 'rose' }], 29: [{ title: 'Tur semineri', tone: 'orange' }],
};
...
export default function PersonalOS() {
  ...
  const renderProjects = () => {
    ...
    return <>
      ...
      <div className="project-summary"><div><span className="project-stat-icon violet"><LayoutGrid size={18}/></span><span><strong>{allProjects.length}</strong><small>Proje havuzu</small></span></div><div><span className="project-stat-icon mint"><Zap size={18}/></span><span><strong>{averageProgress}%</strong><small>Ortalama ilerleme</small></span></div><div><span className="project-stat-icon blue"><Clock3 size={18}/></span><span><strong>{allProjects.filter((project)=>project.stage>0&&project.stage<3).length}</strong><small>Aktif üretim</small></span></div></div>
      <div className="project-thesis"><Sparkles size={15}/><span><strong>Proje filtresi</strong><small>Tasarım + teknoloji + uzay + dinozor + retro + futuristic + görsel üretim + interaktif deneyim.</small></span></div>
  ...
  const renderPrograms = () => (
    <>
      ...
      <div className="program-stack">{programs.map((program)=>{const open=expandedProgram===program.id;const done=programCategories.reduce((sum,cat,catIndex)=>sum+cat.tasks.filter((_,taskIndex)=>state.completed[`program-${program.id}-${catIndex}-${taskIndex}`]).length,0);const total=programCategories.reduce((sum,cat)=>sum+cat.tasks.length,0);return <article key={program.id} className={`surface program-card ${open?'open':''}`}><button className="program-head" onClick={()=>setExpandedProgram(open?null:program.id)}><span className={`program-date ${program.accent}`}><strong>{program.range.split(' ')[0]}</strong><small>{program.range.split(' ').slice(1).join(' ')}</small></span><span className="program-name"><em>{program.status}</em><h2>{program.title}</h2><small><Users size={12}/>{program.people} yolcu</small></span><span className="program-progress"><strong>{Math.max(program.progress,Math.round(done/total*100))}%</strong><i><b style={{width:`${Math.max(program.progress,Math.round(done/total*100))}%`}}/></i><small>{done}/{total} kontrol</small></span><span className="program-chevron"><ChevronDown size={19}/></span></button><div className="program-content"><div className="category-grid">{programCategories.map((category,catIndex)=>{const CategoryIcon=category.icon;const catDone=category.tasks.filter((_,taskIndex)=>state.completed[`program-${program.id}-${catIndex}-${taskIndex}`]).length;return <details key={category.name} open={catIndex===0&&open}><summary><span className={`category-icon c${catIndex}`}><CategoryIcon size={17}/></span><span><strong>{category.name}</strong><small>{catDone}/{category.tasks.length} tamamlandı</small></span><b>{Math.round(catDone/category.tasks.length*100)}%</b><ChevronDown size={14}/></summary><div className="category-tasks">{category.tasks.map((task,taskIndex)=>{const id=`program-${program.id}-${catIndex}-${taskIndex}`;return <button key={id} onClick={()=>toggle(id)} className={state.completed[id]?'completed':''}><span>{state.completed[id]&&<Check size={10}/>}</span>{task}<small>{taskIndex<2?'Bugün':'Bu hafta'}</small></button>})}</div></details>})}</div></div></article>})}</div>
    </>
  );
