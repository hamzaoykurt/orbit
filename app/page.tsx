...
export default function PersonalOS() {
  ...
  const mentorProjectView=(project:Project):MentorProjectView=>{
    ...
    const plan=workspace.planning;const firstTask=remaining[0]||plan?.analysis.firstSteps[0]||'';
    const inferredStage=project.stage===0&&!/fikir|havuz/i.test(project.due)?(/araştır|research|explor|incele/i.test(firstTask)?'Araştırma':'Aktif'):['Fikir','Aktif','Araştırma','Tamamlandı'][project.stage]||'Fikir';
    const stage=plan?lifecycleLabels[plan.lifecycle]:inferredStage;
