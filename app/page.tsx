...
import { findSimilar, mentorContext, parseMentorJson, projectContext, researchContext, serializeMentorDocument } from './mentor-sync';
...
import {
  ...
  Circle, Clock3, Command, Compass, Download, Dumbbell, Eye, Globe2,
...
export default function PersonalOS() {
  ...
  const [mentorFileName,setMentorFileName]=useState('');
  ...
  const readMentorDocument=async(file:File)=>{
    setMentorError('');setMentorParsed(null);setMentorDuplicate(null);setMentorFileName(file.name);
    if(!file.name.toLocaleLowerCase('tr-TR').endsWith('.json')||file.size>256*1024){setMentorError('En fazla 256 KB boyutunda bir JSON belgesi seç.');return;}
    try{const parsed=parseMentorJson(await file.text());if(!parsed){setMentorError('Belgede geçerli bir mentor_response kaydı bulunamadı.');return;}prepareMentorImport(parsed);}catch{setMentorError('JSON belgesi okunamadı. Dosyayı yeniden seç.');}
  };
  const downloadMentorDocument=()=>{
    const generatedAt=new Date(),content=serializeMentorDocument(buildMentorContext(generatedAt),generatedAt),blob=new Blob([content],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');
    link.href=url;link.download=`orbit-mentor-context-${localDateKey(generatedAt)}.json`;link.click();URL.revokeObjectURL(url);setMentorGeneratedAt(generatedAt);notify('Mentor JSON belgesi indirildi.');
  };
  const importFromMentor=()=>{
    ...
    if(mentorParsed.kind==='research'){
      ...
      setModal(null);setMentorFileName('');setMentorParsed(null);go('rebuild');notify('Araştırma, mevcut Research sistemine eklendi.');return;
    }
    ...
    setModal(null);setMentorFileName('');setMentorParsed(null);openProjectDetail(id);notify('Proje, görevleri ve başlangıç rotasıyla eklendi.');
  };
  ...
  const renderProjects = () => {
    ...
    return <Suspense fallback={<p role="status">Projeler açılıyor…</p>}><ProjectsHub projects={projectMetrics} workspaces={state.projectWorkspaces} hasDraft={!!state.projectCreationDraft} onNew={openProject} onOpen={openProjectDetail} onCopy={id=>{const item=metricProjects.find(project=>project.id===id);if(item)void copyText(projectContext(mentorProjectView(item)),'Proje bağlamı kopyalandı.');}}/></Suspense>;
  };
